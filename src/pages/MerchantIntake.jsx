...
const MerchantIntake = () => {
  ...
  // Debug Data State
  const [debugData, setDebugData] = useState(null); // Gemini Raw
  const [localData, setLocalData] = useState(null); // Tesseract Raw
  ...
  // Helper: Prioritize Gemini Value, fallback to Tesseract/User
  ...
  // --- LOGIC: DUAL-ENGINE ANALYSIS (INLINE) ---
  const handleAnalysis = async (file, category, context, officerId = null) => {
    ...
    try {
        ...
        const tessData = tesseractResult.extracted || {}; // Regex Data
        ...
        // 🧠 2. Consensus Logic & State Update
        if (context === 'COMPANY') {
            setCompany(prev => ({
                ...prev,
                file_id: fileId,
  ...
  const submitAll = async () => {
    for (let i = 0; i < officers.length; i++) {
        const o = officers[i];
        if (!o.full_name || !o.role || !o.passport_number || !o.dob) {
    ...
    try {
        ...
        // Log Audit if supported
        if(api.logAudit) await api.logAudit("SUBMIT_APPLICATION", company.merchant_id, `Submitted with ${officers.length} officers`);
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 space-y-8">
                <div className="bg-obsidian-800 p-6 md:p-8 rounded-2xl border border-gray-700 shadow-xl">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                        ...
                        <button onClick={() => setShowDebug(!showDebug)} className="flex items-center gap-2 text-xs text-gold-400 hover:text-white transition-colors border border-gold-500/30 px-3 py-1.5 rounded-full self-end md:self-auto">
                ...
                {officers.map((officer, index) => (
                    <div key={officer.id} className="bg-obsidian-800 p-6 rounded-xl border border-gray-700 relative shadow-xl transition-all">
                        ...
                        <div className="mb-6">
                            ...
                            <select value={officer.role} onChange={e => setOfficers(officers.map(o => o.id === officer.id ? { ...o, role: e.target.value } : o))} className={`w-full bg-obsidian-900 border rounded-lg p-3 text-white focus:outline-none focus:border-gold-400 appearance-none cursor-pointer transition-all ${!officer.role ? 'border-gold-500/50 shadow-[0_0_15px_rgba(212,175,55,0.15)] animate-pulse' : 'border-gray-700'}`}>
                                ...
                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-gray-700/50">
                                        <div className="col-span-1">
                                            ...
                                            <select value={officer.doc_type} onChange={(e) => setOfficers(officers.map(o => o.id === officer.id ? { ...o, doc_type: e.target.value } : o))} className="w-full bg-obsidian-900 text-xs border border-gray-700 rounded mb-3 p-2 text-white focus:border-gold-400 outline-none cursor-pointer">
