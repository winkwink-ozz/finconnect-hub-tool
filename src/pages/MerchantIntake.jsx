// === START FILE: src/pages/MerchantIntake.jsx ===
    ...
    try {
      ...
      forms.forEach(form => {
        ...
        form.schema.forEach(q => {
          if(q.type === 'table') {
            // ✅ NEW: Apply Defaults Logic
            initAnswers[form.id][q.id] = Array(q.initialRows).fill(null).map((_, idx) => {
                // Check if admin has defined a default row for this index
                const defaultRow = (q.defaults && q.defaults[idx]) ? q.defaults[idx] : {};
                const rowObj = {};
                q.columns.forEach(col => {
                    // Use default value if it exists, otherwise empty string
                    rowObj[col] = defaultRow[col] || "";
                });
                return rowObj;
            });
          }
                        ...
                        <div key={q.id}>
                          ...
                          {q.type === 'table' && (
                            <div className="overflow-x-auto rounded-lg border border-gray-700">
                              <table className="w-full text-left border-collapse">
                                <thead>
                                  <tr className="bg-black/40 text-gray-400 text-xs uppercase">
                                    {q.columns.map((col, idx) => ( <th key={idx} className="p-3 border-b border-gray-700">{col}</th> ))}
                              ...
                              <button onClick={() => addTableRow(form.id, q.id, q.columns)} className="w-full py-2 text-xs text-gold-400 hover:bg-gold-500/10 flex items-center justify-center gap-1">
...
// === END FILE: src/pages/MerchantIntake.jsx ===
