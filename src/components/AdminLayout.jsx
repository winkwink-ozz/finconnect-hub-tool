...
const AdminLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-obsidian-900 text-gray-100 font-sans flex flex-col md:flex-row">
      ...
      {/* 🖥️ DESKTOP SIDEBAR */}
      <aside className="hidden md:flex flex-col w-64 bg-obsidian-800 border-r border-gray-700 h-screen sticky top-0">
        <div className="p-6 flex items-center gap-3 border-b border-gray-700">
          ...
          <div>
            <h1 className="font-bold text-white tracking-tight">Admin Portal</h1>
            <p className="text-[10px] text-gold-400 uppercase tracking-widest">FinConnect Hub</p>
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-2">
          {navItems.map((item) => (
            <Link key={item.path} to={item.path}>
              <div className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isActive(item.path) ? 'bg-gold-500 text-black font-bold shadow-lg shadow-gold-500/20' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                <item.icon size={20} />
      ...
      {/* 📱 MOBILE HEADER */}
      <header className="md:hidden bg-obsidian-800 border-b border-gray-700 p-4 flex items-center justify-between sticky top-0 z-50">
      ...
      {/* 📱 MOBILE BOTTOM NAV */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-obsidian-800 border-t border-gray-700 p-2 flex justify-around z-50 pb-safe">
        {navItems.map((item) => (
